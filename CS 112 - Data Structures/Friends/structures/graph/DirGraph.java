...
public class DirGraph<T> {
	...
	private Vertex<T> vertexFromNeighbor(Neighbor nghbr) {
		return this.adjlists.get(nghbr.vertexNumber);
	}
	...
	public int getConnectedComponents() {
		...
		for(int v = 0; v< adjlists.size(); v++){
			if(number[v] == 0){
				j++;
				number[v]=j;
				ArrayList<Integer> Russell = new ArrayList<Integer>();
				ArrayList<Integer> Jared = new ArrayList<Integer>();
				Russell.add(v);
				while(!Russell.isEmpty()){
					int x = Russell.remove(Russell.size()-1);
					if(!Jared.contains(x)){
						Jared.add(x);
						for(int i=0; i< adjlists.size(); i++){
	...
	public UndirGraph<T> removeVertexFromGraph(int vrtxNum) {
		...
		vertexCount = subgraph.numberOfVertices();
	...
	public T vertexInfoOf(int vertexNumber) {
		Vertex<T> JP = adjlists.get(vertexNumber);
		return JP.info;
	}
	...
	public boolean containsEdge(int vertexNumber, Neighbor nbr) {
		Vertex<T> RJ = adjlists.get(vertexNumber);
		return RJ.neighbors.contains(nbr);
	}
	 ...
	 */
	public void addEdge(int vertexNumber, Neighbor nghbr) {
		Vertex<T> fromVertex = adjlists.get(vertexNumber);
		if (!fromVertex.neighbors.contains(nghbr)) {
			fromVertex.neighbors.add(nghbr);
		}
	}
	 ...
	 */
	public Neighbor firstNeighbor(int theVertexNumber) {
		Vertex<T> v = adjlists.get(theVertexNumber);
	 ...
	 */
	public Neighbor nextNeighbor(int theVertexNumber) {
		Vertex<T> v = adjlists.get(theVertexNumber);
